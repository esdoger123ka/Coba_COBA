...
async def order_selected(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    await query.message.reply_text("Masukkan Service Number (No Inet/Voice/Site/dll):")
...
async def order_query(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if len(matches) == 1:
        ...
        await update.message.reply_text("Masukkan Service Number (No Inet/Voice/Site/dll):")
        return SERVICE_NUMBER
...
async def service_number(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    context.user_data["service_number"] = update.message.text.strip()
    await update.message.reply_text("Masukkan WO Number (SC/WO):")
    return WO_NUMBER


async def wo_number(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    context.user_data["wo_number"] = update.message.text.strip()
    await update.message.reply_text("Masukkan Ticket ID (No Tiket):")
    return TICKET_ID
...
async def tech_selected(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    await query.message.reply_text("Masukkan Workzone:")
    return WORKZONE
...
async def tech2_decide(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    await query.message.reply_text("", reply_markup=_unit_keyboard(units, 0, "t2"))
    return TECH2_UNIT


async def workzone(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    context.user_data["workzone"] = update.message.text.strip()
    await update.message.reply_text("Masukkan Keterangan (atau ketik /skip):")
    return KETERANGAN
